/**
 * FaultyTerminalBackground – WebGL CRT/Terminal background (OGL)
 * Scanlines, glitch, curvature, mouse reaction, page-load animation.
 */
...
export interface FaultyTerminalProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  brightness?: number;
  /** Background color (hex) where the pattern is empty */
...
float pattern(vec2 p, out vec2 q, out vec2 r) {
  float rVal = fbm(p);
  float qVal = fbm(p + vec2(1.2, 0.8));
  q = vec2(qVal, qVal);
  r = vec2(rVal, rVal);
  return fbm(p + rVal * 0.5 + qVal * 0.35);
}
...
vec3 getColor(vec2 p){
    ...
    float bar = uScanlineIntensity < 0.001 ? 1.0 : (step(mod(p.y + time * 20.0, 1.0), 0.2) * 0.4 + 1.0) * uScanlineIntensity;
    
    float displacement = uFlickerAmount < 0.001 ? 0.0 : displace(p);
    p.x += displacement;
    if (uGlitchAmount != 1.0 && uFlickerAmount >= 0.001) {
    ...
    float sum = digit(p + vec2(-off, 0.0)) + digit(p + vec2(off, 0.0)) + digit(p + vec2(0.0, -off)) + digit(p + vec2(0.0, off));
...
void main() {
    ...
    col = mix(uBackground, col, smoothstep(0.0, 0.12, lum));
...
export function FaultyTerminalBackground({
  ...
  useEffect(() => {
    ...
    const iResolution = new Float32Array([
      gl.canvas.width,
      gl.canvas.height,
    ...
    function resize() {
      ...
      const cw = ctn.offsetWidth;
      const ch = ctn.offsetHeight;
      if (cw <= 0 || ch <= 0) return;
      const containerAspect = cw / ch;
      let fitW: number;
      let fitH: number;
      if (containerAspect >= REF_ASPECT) {
        fitW = cw;
        fitH = cw / REF_ASPECT;
      } else {
        fitH = ch;
      ...
      renderer.setSize(Math.round(fitW), Math.round(fitH));
  ...
  return (
    <div
      ref={containerRef}
      className={`faulty-terminal-bg ${className}`.trim()}
      style={style}
      {...rest}
    />
  );
